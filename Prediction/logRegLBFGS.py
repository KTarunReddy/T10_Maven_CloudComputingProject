...
#training the model with Logistic Regression with LBFGS - L1
logisticRegressionModel = LogisticRegressionWithLBFGS.train(parsedTrainData)

# evaluating the model on testing data
labelsAndPreds = parsedTestData.map(lambda p: (logisticRegressionModel.predict(p.features)))
labelsAndPreds.saveAsTextFile("D:/CloudComputing/CourseProject/Data/Merge/Reduction/FeatureGeneration/Prediction/predictionOutput/predictionFile")
#testingErr = labelsAndPreds.filter(lambda (v, p): v != p).count() / float(parsedTestData.count())
#print("Testing Error = " + str(testingErr))

# saving the model
#logisticRegressionModel.save(sc, "/Users/annu/Desktop/IDS/LRModel")
#logisticRegressionModel = LogisticRegressionModel.load(sc, "/Users/annu/Desktop/IDS/LRModel")
